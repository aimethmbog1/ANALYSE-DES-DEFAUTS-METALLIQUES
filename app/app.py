...
st.set_page_config(
    page_title="ANALYSE DES DÉFAUTS MÉTALLIQUES",
    page_icon="🧠",
    layout="wide"
)
...
for key in ["filtered_preds", "class_name", "confidence"]:
...
with st.sidebar:
    ...
    st.markdown("---")
    st.info("Aide à la décision.\nValidation humaine requise.")

# =========================
# MAIN UI
# =========================

st.title("🧠 ANALYSE DES DÉFAUTS MÉTALLIQUES")
st.markdown("Analyse automatisée par Deep Learning")
...
with tab_analysis:
    ...
    if not image_loaded:
        ...
        with col_res:
            ...
            st.session_state.filtered_preds = {
                cls: float(raw_preds[ALL_CLASSES.index(cls)])
                for cls in selected_classes
            }

            st.session_state.class_name = max(
                st.session_state.filtered_preds,
                key=st.session_state.filtered_preds.get
            )

            st.session_state.confidence = st.session_state.filtered_preds[
            ...
            <div class="card {css}">
                ...
                <p><strong>Classe :</strong> {st.session_state.class_name}</p>
                <p><strong>Confiance :</strong> {st.session_state.confidence:.2%}</p>
            </div>
            """, unsafe_allow_html=True)
...
with tab_details:
    ...
    if st.session_state.filtered_preds is None:
        ...
        ax.barh(
            list(st.session_state.filtered_preds.keys()),
            list(st.session_state.filtered_preds.values())
        )
        ax.set_xlim(0, 1)
...
with tab_export:
    ...
    if st.session_state.filtered_preds is None:
        ...
        result = {
            "predicted_class": st.session_state.class_name,
            "confidence": st.session_state.confidence,
            "used_classes": selected_classes,
            "probabilities": st.session_state.filtered_preds
        }
